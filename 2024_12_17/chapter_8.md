...
##### 주요 흐름
##### 1. 스레드 상태가 유효한지 확인한다.
##### => PyThreadState 구조체가 제대로 생성되고 스레드와 연결되어 있음
##### => 현재 실행 중인 인터프리터에 속해있음
##### => GIL을 획득한 상태 (gil은 한 번에 하나의 스레드만 실행하도록 관리)
##### => 스택 오버플로우가 x
##### => 스레드 중단이 없음
...
##### 3. 프레임 객체를 생성한다.
##### => 메모리에 새로운 프레임 객체를 생성
##### => 생성된 프레임 객체는 현재 스레드 상태에 연결
##### 4. 인자와 변수를 처리한다.
##### => 코드에 전달된 함수 인자, 키워드 인자 초기화
##### => 코드 실행에 필요한 임시 변수나 값도 메모리에 할당
##### 5. 제너레이터 객체와 코루틴 객체를 처리한다.
##### => 제너레이터 : 프레임 객체를 멈출 수 있도록 상태를 저장
##### => 다음 실행 지점을 추적 (yield)
##### => 코루틴 : 비동기 코드 실행을 위한 코루틴 객체를 초기화
##### 6. 프레임을 실행하고 결과를 저장한다.
##### => 바이트 코드를 하나씩 가져와 처리
##### => 프레임의 스택이나 로컬 변수에 실행 결과를 저장
##### 7. 프레임 객체의 참조 수를 확인하고 관련 객체들을 해제한다.
##### => 프레임 실행이 끝나면 참조 카운트를 확인 참조 카운트가 0 이라면 메모리에서 해제
##### => 로컬변수, 임시 객체, 스택등을 메모리에서 해제
##### => 제너레이터나 코루틴 같은 객체는 상태를 유지하기 위해 일부 참조를 남김
##### 8. 결과를 반환한다.
##### => 반환 값은 부모 프레임으로 전달
##### => 프레임이 완전히 종료되면 해당 프레임 객체는 메모리에서 해제
